...
from myapp3.models import Autor, Post
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(1, count + 1):
            author = Autor(name=f'Author_{i}', email=f'mail{i}@mail.ru')
            author.save()
            for j in range(1, count + 1):
                post = Post(title=f'Title-{j}', content=" ".join(choices(text, k=64)), autor=author)
                post.save()